...
import { Plus } from "@/components/icon/Plus.tsx";
import { addNotification } from "@/libs/notification.ts";
import {
  loadActiveSheet,
  loadSheets,
  saveActiveSheet,
  saveSheets,
} from "@/libs/localStorage.ts";
import { FilePlus } from "@/components/icon/FilePlus.tsx";
import { Folder } from "@/components/icon/Folder.tsx";
import { HelpCircle } from "@/components/icon/HelpCircle.tsx";
import { Edit3 } from "@/components/icon/Edit-3.tsx";
import { Trash2 } from "@/components/icon/Trash-2.tsx";
import { Sheet } from "@/libs/localStorage.ts";
...
export default class WhiteBoardPage
  extends Component<WhiteBoardPageProps, WhiteBoardPageState> {
  ...
  addBeatOnClick(barIndex: number, beatIndex: number) {
    return () => {
      if (
        this.state.sheets[this.state.activeSheet].bars[barIndex].beats.length <
          6
      ) {
        addBeatModal.value = {
          ...addBeatModal.value,
  ...
  addBar() {
    if (this.state.sheets[this.state.activeSheet].bars.length > 60) {
      ...
      this.state.sheets[this.state.activeSheet].bars.push({
        totalBeats: 4,
        beats: [],
      });
      this.saveSheets();
  ...
  renameSheet() {
    this.state.sheets[this.state.activeSheet].title =
      renameSheetModal.value.text;
  ...
  saveSheets() {
    this.state.sheets[this.state.activeSheet].lastEdit = (new Date())
      .toString();
  ...
  deleteSheet(i: number) {
    ...
    if (
      confirm(
        `Do you want to delete whiteboard "${
          this.state.sheets[i].title
        }"? You cannot recover the deleted whiteboard.`,
      )
    ) {
      ...
      <Fragment>
        <div className={"whiteboard-container"}>
          <div className={"menu-container"}>
            <div
              onClick={() => {
                newSheetModal.value = { ...newSheetModal.value, show: true };
              }}
            >
              <span>New</span>
              <FilePlus size={18} />
            ...
            <div
              onClick={() => {
                renameSheetModal.value = {
                  ...renameSheetModal.value,
                  show: true,
                  text: this.state.sheets[this.state.activeSheet].title,
                };
              }}
            >
              <span>Rename</span>
            ...
            <div
              onClick={() => {
                openSheetModal.value = { ...openSheetModal.value, show: true };
              }}
            >
              <span>Open</span>
              <Folder size={18} />
              ...
              >
                ...
                <span
                  className={[
                    ...
                    "color-" +
                    (simpleKeys.indexOf(
                      this.state.sheets[this.state.activeSheet].key,
                    ) + 1),
                  ].join(" ")}
                >
                  {this.state.sheets[this.state.activeSheet].key}
              ...
              >
                ...
                <span
                  className={[
                    "mode",
                    "color-" +
                    (this.state.sheets[this.state.activeSheet].mode + 1),
                  ].join(" ")}
                >
          ...
          <div className={"sheet-container"}>
            {this.state.sheets[this.state.activeSheet].bars.map((
              bar,
              barIndex,
            ) => (
              <div className={["bar", this.state.edit ? "edit" : ""].join(" ")}>
                {this.state.edit && (
                  <PlusCircle
                    size={18}
                    onClick={this.addBeatOnClick(barIndex, 0)}
                  />
                )}
                {bar.beats.map((beat, beatIndex) => (
                  <Fragment>
                    ...
                    {this.state.edit && (
                      <PlusCircle
                        size={18}
                        onClick={this.addBeatOnClick(barIndex, beatIndex + 1)}
                      />
                    )}
                  </Fragment>
                ))}
        ...
        <Modal
          show={rootKeyModal.value.show}
          setShow={(show) => {
            rootKeyModal.value = { show };
          }}
        >
          <div className="rootKey-modal">
            <KeySelector
              link={false}
              selectedKey={this.state.sheets[this.state.activeSheet].key}
              setKey={(key) => {
            ...
            {modesTable.map((mode, i) => (
              <div
                className={[
                  "mode-item",
                  "color-" + (i + 1),
                  i == this.state.sheets[this.state.activeSheet].mode
                    ? "active"
                    : "",
                ].join(" ")}
                onClick={() => {
        ...
        <Modal
          show={addBeatModal.value.show}
          setShow={(show) => {
            addBeatModal.value = { ...addBeatModal.value, show };
          }}
        >
          <div className="addBeat-modal">
            <h1>Select a tonic:</h1>
            <div className={"rome-container"}>
              {modesTable[this.state.sheets[this.state.activeSheet].mode].rome
                .map((rome, i) => (
                  <span
                    className={[
                      i == addBeatModal.value.activeRome ? "active" : "",
                    ].join(" ")}
                    onClick={() => {
                      addBeatModal.value = {
                        ...addBeatModal.value,
            ...
            <div className={"chord-container"}>
              {(() => {
                ...
                return chords.map((chord) => (
                  <span
                    className={["color-" + (simpleKeys.indexOf(key) + 1)]
                      .join(" ")}
                    onClick={() => {
                      this.addBeat(chord, chord.shortName, "[lyrics]");
                    }}
                  >
                    {chord.shortName}
                  </span>
            ...
            <h2>
              ...
              <a
                onClick={() => {
                  this.addBeat(undefined, "", "[lyrics]");
                }}
              >
                Insert an empty chord
              </a>{" "}
              and you and always edit it later.
            </h2>
          </div>
        </Modal>
        <Modal
          show={newSheetModal.value.show}
          setShow={(show) => {
            newSheetModal.value = { ...newSheetModal.value, show };
          }}
        >
          <div className={"newSheet-modal"}>
            ...
            <input
              ...
              placeholder="Name"
              maxLength={30}
              value={newSheetModal.value.text}
              onChange={(e) => {
                newSheetModal.value.text = (e.target as HTMLInputElement).value;
              }}
            />
            <div>
              <button
                ...
                className={"color-5"}
                onClick={() => {
                  newSheetModal.value = { ...newSheetModal.value, show: false };
                }}
              >
                Cancel
              </button>
        ...
        <Modal
          show={renameSheetModal.value.show}
          setShow={(show) => {
            renameSheetModal.value = { ...renameSheetModal.value, show };
          }}
        >
          <div className={"renameSheet-modal"}>
            ...
            <input
              ...
              placeholder="Name"
              maxLength={30}
              value={renameSheetModal.value.text}
              onChange={(e) => {
                renameSheetModal.value.text =
        ...
        <Modal
          show={openSheetModal.value.show}
          setShow={(show) => {
            openSheetModal.value = { ...openSheetModal.value, show };
          }}
        >
          <div className={"openSheet-modal"}>
            {this.state.sheets.map((sheet, i) => (
              <div
                className={[
                  "sheet-item",
                  "color-" + (simpleKeys.indexOf(sheet.key) + 1),
                  i == this.state.activeSheet ? "active" : "",
                ].join(" ")}
                onClick={() => {
              ...
              >
                <div className={"info-container"}>
                  <div className={"title"}>
                    ...
                    <h2>
                      <b>Last edited:</b>{" "}
                      {new Date(sheet.lastEdit).toLocaleString()}
                    </h2>
                  </div>
