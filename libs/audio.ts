import { Keys, Octave } from "@/libs/key.ts";
...
class Piano {
  ...
  constructor() {
    ...
    if (this.context.decodeAudioData.length !== 1) {
      const originalDecodeAudioData = this.context.decodeAudioData.bind(
        this.context,
      );
  ...
  async loadSample(octave: Octave): Promise<AudioBuffer> {
    if (!this.context) {
      throw new Error("AudioContext is not init");
    }

    const sample = this.samples[octave];
    if (sample === undefined) {
      this.samples[octave] = "loading";
      const response = await fetch(
        `/audio/samples_piano_F${octave}.${this.extension}`,
      );
